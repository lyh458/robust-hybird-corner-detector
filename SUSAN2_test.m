...
img_tmp(4:nrow_tmp-3,4:ncol_tmp-3) = img; %扩展图像边缘3个像素,周边3个像素为0

% t=20;   %模板中心像素灰度和周围灰度差别的阈值，自己设置
usan=[]; %当前像素和周围在像素差别在t以下的个数
%这里用了37个像素的模板
for i=4:nrow_tmp-3         
    for j=4:ncol_tmp-3
        tmp=img_tmp(i-3:i+3,j-3:j+3);   %先构造7*7的模板，49个像素
        cnt=0;
        for p=1:7
            for q=1:7
                if (p-4)^2+(q-4)^2<=12  %在其中筛选，最终模板类似一个圆形
                    %   usan(k)=usan(k)+exp(-(((img_tmp(i,j)-tmp(p,q))/t)^6));
                    if abs(img_tmp(i,j)-tmp(p,q))<t  %判断灰度是否相近，t是自己设置的
                        cnt=cnt+1;
                    end
                end
            end
...
g=c*max(usan); %确定角点提取的数量，值比较高时会提取出边缘，自己设置
for i=1:length(usan)
    if usan(i)<g
        usan(i)=g-usan(i);
    else
