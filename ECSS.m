function [cout,marked_img,angle]=ECSS(varargin)
...
%                     ang_temp = 360-ang;
%                 end
%                 if ang_temp>T_angle || ang>=360
%                     flag(j)=0;
%                 end
%                 output=[j,T_angle,ang,flag(j)]
            end
            ...
            if size(extremum,2)==0
                ...
                angle_temp = angle_temp(find(flag~=0));
            end
        end
        
        extremum=extremum-W;
        extremum=extremum(find(extremum>0 & extremum<=L));
        angle_temp = angle_temp(find(extremum>0 & extremum<=L));
