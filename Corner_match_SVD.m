% 基于奇异值分解的角点匹配法
function  U =Corner_match_SVD( I,J,A,B)
% input:
% I - original image 1,
% J - original image 2,
% A - coordinates of corners in image I,
% B - coordinates of corners in image J.
% output:
% U - 
...
G = zeros(cnt1,cnt2);
for s = 1:cnt1
    u1 = A(s,1);v1 = A(s,2); % 得到角点A的x、y坐标
    for t = 1:cnt2
        u2 = B(t,1);v2 = B(t,2); % 得到角点B的x、y坐标
        su = 0;sum1=0;sum2=0;
        for k = 1:15
            for l = 1:15
                m1 = u1+k-8;n1 = v1+l-8;
        ...
        C(s,t) = su/(15*15*(sqrt(sum1*sum2))); % 归一化互相关
